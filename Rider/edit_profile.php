...
if(!isset($_SESSION['username']) || $_SESSION['role']!="rider"){
...
if(isset($_POST['submit'])){
    $userID = $_POST['userID'];
    $R_licensetype = $_POST['R_licensetype'];
    $R_available = $_POST['R_available'];


    $sql = "UPDATE rider SET R_licensetype='$R_licensetype', R_available='$R_available' WHERE userID='$userID'";

    $result = mysqli_query($conn, $sql);

    if($result){
        header("Location: view_profile.php?msg=Record Updated Successfully!!!");
    }
    else
...
<body class="body-with-footer">
    <!-- -----------Header----------- -->
    <div class="welcome">
<h4 class="text-align:right"><i class="fas fa-motorcycle"></i><?=$_SESSION['username'] ?></h4>
</div>
<div class="nav-container">
<a href="index.php"><img id="logo" src="../media/onp logo.png" alt="Logo"></a>
  <nav>
    <a href="rider.php">HOME</a>
    <a href="#">USER</a>
    <a href="#">ORDER</a>
    <a href="#">PAYMENT</a>
    <a href="#">APPROVAL</a>
...
<div class="container">
    <div class="text-center mb-4">
        ...
        <p class="text-muted">Please Click Update to save the changes</p>
    </div>
    
    <?php 
if (!empty($_GET['userID'])) {
    $userID = $_GET['userID'];
    $stmt = $conn->prepare("SELECT * FROM rider WHERE userID = ? LIMIT 1");
...
?>
    ...
    <div class="row mb-3">
        <div class="col">
            ...
            <input type="text" class="form-control" name="R_licensetype" 
            value="<?php echo $row['R_licensetype'] ?>">
        </div>
    </div>

    <div class="mb-3">
        <label class="form-lebel">User's Address</label>
        <input type="address" class="form-control" name="R_available" 
        value="<?php echo $row['R_available'] ?>">
    </div>
    <div>
        <button type="submit" class="btn btn-success" name="submit">UPDATE</button>
        <a href="rider.php" class="btn btn-danger">CANCEL</a>
    </div>
    </form>
    </div>
</div>
